...
class CafeForm(FlaskForm):
    ...
    Location_URL=StringField('Cafe Location on google map', validators=[DataRequired()])
    open_time=StringField('Opening Time e.g 8am', validators=[DataRequired()])
    closing_time=StringField('Closing Time e.g 5:30pm', validators=[DataRequired()])
    coffee_rating=SelectField('Coffee Rating',
                        choices=[ "☕️" * (i + 1) for i in range(5)],
                        validators=[DataRequired()])
    wifi_rating=SelectField('WIFI Strength Rating',
                        choices = ['✘'] + ["💪" * (i + 1) for i in range(5)],
                        validators=[DataRequired()])
    power_outlet_rating=SelectField('Power Outlet Availability',
                        choices = ['✘'] + ["🔌" * (i + 1) for i in range(5)],
                        validators=[DataRequired()])
